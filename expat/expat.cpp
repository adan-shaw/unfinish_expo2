...
//编译:
//g++ -o x ./expat.cpp -lexpat

//https://www.cnblogs.com/274914765qq/p/4575438.html
//连带dom xml 一起搞了, 反正都这个地步了
...
/*
	...
	(ps: expat 不需要装载完整的xml 文档, 可以边装载边解析.
			 读到一个<A> </A>就提取一个元素, 但是expat 不提供xml 文档生成.
	...
	Expat是一个C语言编写的XML解析库.
	它是一个面向流的解析器, 应用程序可以对解析中发现的XML元素注册处理函数.
...
//every element Parse and work-start
//expat 每解析一个xml 元素, 就调用一次回调函数, start and end
...
//every element Parse and work-end
//expat 每解析一个xml 元素, 就调用一次回调函数, start and end
...
int main(int argc, char *argv[]){
	...
	//打印文件信息, 检查文件大小
	tmp = stat_check(argv[1],&file_max);
	if(tmp == false){
	...
	//检查解析的xml 文件是否超出限制
	if(file_max > FILE_MAX_PARSER_SAX_XML){
		printf("file_max=%d > FILE_MAX_PARSER_SAX_XML=%d, EXCEEDS LIMIT!",\
				file_max, FILE_MAX_PARSER_SAX_XML);
		return -1;
	}
